...
struct ProcessTableReducer {
    ...
    mutating func update(
        ...
        for sample in samples {
            ...
            // CPU% = Δcpu_ns / Δwall_ns / core count (spec §4.1). New rows: 0.
            var cpuPercent = 0.0
            if let previous, let dtSeconds, sample.cpuNanoseconds >= previous.cpuNanoseconds, coreCount > 0 {
                let delta = Double(sample.cpuNanoseconds - previous.cpuNanoseconds)
                cpuPercent = delta / (dtSeconds * 1_000_000_000) / Double(coreCount) * 100
            }
